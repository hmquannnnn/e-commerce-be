...
func (h *OrderHandler) AdminUpdateStatus(c *gin.Context) {
	...
	if err := h.orderService.AdminUpdateStatus(c.Request.Context(), orderID, model.OrderStatus(req.Status)); err != nil {
		...
		if errors.Is(err, service.ErrInvalidStatusTransition) {
			respondError(c, http.StatusConflict, "INVALID_STATUS_TRANSITION", "Order cannot transition to the requested status")
			return
		}
...
// InternalMarkPaid is the only entrypoint payment-service uses to flip an order
// to PAID. The transition is guarded inside the service layer so a late or
// retried webhook can never overwrite a CANCELLED / DELIVERING / DELIVERED order.
func (h *OrderHandler) InternalMarkPaid(c *gin.Context) {
	orderID, err := uuid.Parse(c.Param("id"))
	if err != nil {
		respondError(c, http.StatusBadRequest, "INVALID_ID", "Invalid order ID")
		return
	...
	if err := h.orderService.MarkOrderPaid(c.Request.Context(), orderID); err != nil {
		if errors.Is(err, service.ErrOrderNotFound) {
			respondError(c, http.StatusNotFound, "ORDER_NOT_FOUND", "Order not found")
			return
		}
		...
		respondError(c, http.StatusInternalServerError, "INTERNAL_ERROR", "Failed to mark order paid")
		return
	}
...
func (h *OrderHandler) InternalGetOrder(c *gin.Context) {
	orderID, err := uuid.Parse(c.Param("id"))
	if err != nil {
		respondError(c, http.StatusBadRequest, "INVALID_ID", "Invalid order ID")
		return
	...
	respondOK(c, "Order retrieved", internalOrderResponse{
		ID:         order.ID,
		UserID:     order.UserID,
